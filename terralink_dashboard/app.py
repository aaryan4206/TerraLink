...
class TerraLinkApp:
                ...
                with self.db:
                    self.db.execute(
                        ...
                        INSERT INTO readings(
                            ...
                            temperature,humidity,pressure,gas,rssi,snr
                        )
                        VALUES(?,?,?,?,?,?,?,?,?)
                        """,
                        (
                            data["timestamp"], data["node_id"],
                            data["packet_number"], data["temperature"],
                            data["humidity"], data["pressure"],
                            data["gas"], data["rssi"], data["snr"]
                        )
                    )
    ...
    def reading(self, data):
        for key in ["temperature", "humidity", "pressure", "gas", "rssi", "snr"]:
            value = data[key]
