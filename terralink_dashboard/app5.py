...
def add_reading(node_id, temperature, humidity, pressure, gas,
                battery_voltage, rssi, snr, timestamp=None):
    timestamp = timestamp or datetime.now().isoformat(timespec="seconds")
    conn = get_conn()
    conn.execute("""
        INSERT INTO readings
        (node_id, timestamp, temperature, humidity, pressure, gas,
...
def seed_demo_data():
    ...
    for node_num in range(1, 4):
        ...
        for i in range(72):
            ...
            add_reading(node, temp, hum, pressure, gas, battery, rssi, snr, t.isoformat(timespec="seconds"))
...
n3.metric("Packets stored", f"{len(df):,}")
